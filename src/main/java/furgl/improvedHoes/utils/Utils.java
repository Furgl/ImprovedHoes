...
public class Utils {
	...
	public static Map<Block, BlockState> TILLED_BLOCKS;
	...
	static {
		...
		try {
			TILLED_BLOCKS = (Map<Block, BlockState>) tilledBlocksField.get(null);
		}
		catch (Exception e) {
			e.printStackTrace();
		}
		...
		if (!world.isClient && TILLED_BLOCKS.containsKey(originalBlock)) {
			for (int x=-range.radius; x<=range.radius; x++)
				for (int z=-range.radius; z<=range.radius; z++) {
					...
					BlockState newState = TILLED_BLOCKS.get(block);
					if (newState != null) {
		...
		if (block instanceof CropBlock && ((CropBlock) block).isMature(state)) {
			...
			if (replant && !player.isCreative()) {
				...
				if (itemConvertible != null) {
					...
					if (player.inventory.remove(inventoryStack -> inventoryStack != null && inventoryStack.getItem() == item, 1, player.inventory) != 1)
						replant = false;
				}
			}
			// break block
			...
			if (replant && Config.replantOnHarvest)
				world.setBlockState(pos, ((CropBlock) block).getDefaultState(), 11);
			else
